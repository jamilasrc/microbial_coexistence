...
## DEPRECATED
#@njit
#def apply_cnvrt_dnmcs(t,spec,chem,
                      df_by_spec,a_r_chunked,
    ...
    #    #initiliase chunked data
        ...
        df_chunk = df_by_spec[chunk[0]:(chunk[-1]+1)] # might need to add a +1
        
        ancest_parm_chunk = parms_to_evolve[chunk,:]
        
        dense_matrix = np.vstack(([sdf_to_dense_c(df,min_param,max_param,ancest_parm) 
                                      for df,ancest_parm in zip(df_chunk,ancest_parm_chunk)]))
        
        
        #t_s1 = time.time()
       # dense_matrix = np.zeros((len(s_chunk),ancest_parm_chunk.shape[1]))
        
        #for s_i in range(len(chunk)):
        #for df, ancest_parm, chnk in zip(df_chunk,ancest_parm_chunk,chunk):   
        
            #df = df_chunk[s_i]
            #ancest_parm = ancest_parm_chunk[s_i]
            #s_position = chunk[s_i] - chunk[0]
       #     s_position = chnk - chunk[0]
            
       #     dense_matrix[(s_position + df.shape[0] - 1),:]  = sdf_to_dense_c(df,min_param,max_param,ancest_parm)
        #t_e1 = time.time()
        
       
        #t_s2 = time.time()
        #dense_matrix = np.stack(([sdf_to_dense_c(df,min_param,max_param,ancest_parm) 
#                                      for df,ancest_parm in zip(df_chunk,ancest_parm_chunk)]))
        #dense_matrix = dense_matrix.reshape((df_chunk.shape[0],ancest_parm_chunk.shape[1]))
        #t_e2 = time.time()
...
## DEPRECATED
#@njit
#def sdf_to_dense_c(df,min_param,max_param,ancest_parm):
    ...
    cols = df[:,0].astype(np.int32)
    dense_mat[np.arange(df.shape[0]),cols] = df[:,1] 
    dense_mat[0,:] = ancest_parm
    
    final_dm = np.zeros((dense_mat.shape))
    
    for c in range(dense_mat.shape[1]):
        
        final_dm[:,c] = np.cumsum(dense_mat[:,c])
    
    return final_dm


#@njit
#def cnvrt_dnmcs_numba(t,spec,chem,
                      df_by_spec,a_r_chunked,
                      parms_to_evolve,e_parm_ind,fixed_parm,f_parm_ind,min_param,max_param):
    ...
    for i in range(len(a_r_chunked)-1):
        ...
        df_chunk = df_by_spec[chunk[0]:end_of_chunk] # might need to add a +1
        ...
        for df, ancest_parm, chnk in zip(df_chunk,ancest_parm_chunk,chunk):   
            ...
            dense_matrix[s_position:(s_position + df.shape[0]),:]  = sdf_to_dense_c(df,min_param,max_param,ancest_parm)
        
        dS_chunk, dC_chunk = dynamics_calc_chunk2(t,
                                           s_chunk,dense_matrix,e_parm_ind,fixed_parm,f_parm_ind,
                                           chem)
...
#    kSat = fixed_parm[f_parm_ind[1]]
#     
#    dS, dC = interaction_chunk2(s_chunk,chem,r0,K,alpha,beta,rho_plus,rho_minus,kSat)
#    
#    return dS, dC

#@njit
#def interaction_chunk2(s_chunk, chem, r0, K, alpha, beta, rho_plus, rho_minus, kSat):
#
#    '''
#    
#
...
def species_dynamics_evo_bnchmrk(dtau,
    ...
    ########################## Chunked dynamics, numba ########################
    
    df_by_spec = np.array_split(biomass_table.values,ancest_row[1:]) #converts to numpy array (kinda)
    #df_by_spec = List(df_by_spec) # for numba, numba.typed.List
    
    #a_r_chunked = List(a_r_chunked) # for numba, numba.typed.List
    ...
    for rep in range(200):
        ...
        dynamics_sol = solve_ivp(fun=dDynamics_dt_numba, t_span=[0,dtau], method = 'LSODA',
                               rtol = 10e-9, y0 = np.concatenate((biomass_table['Number'].values,cMed)),
                               args = (spec_len,df_by_spec,a_r_chunked,parms_to_evolve,
                                       e_parm_ind,fixed_parm,f_parm_ind,min_param,max_param))
        t4 = time.time()
...
def dDynamics_dt_numba(t,y,
    ...
    dSCdt = cnvrt_dnmcs_numba(t,spec,chem,
                          df_by_spec,a_r_chunked,
                          parms_to_evolve,e_parm_ind,fixed_parm,f_parm_ind,min_param,max_param)
    dSCdt = np.reshape(dSCdt,(dSCdt.shape[0],))
